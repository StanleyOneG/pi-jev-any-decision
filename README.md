...
- up to 12 compact role capability summaries with explicit purpose (`execution`, `research`, `review`), availability, suitability, authorization and tool IDs;
- 1–8 execution `options`, including exactly one admissible `direct` baseline. A delegated option names its role(s) and required tools; describe serial, parallel, mixed, partial, consultation or review work as applicable. Each option supplies an ID, kind, English summary, evidence, verification criteria, authorization, write conflict, task suitability, context dependency, decisions recorded (`documents`, `conversation`, `both`, `unknown`), handoff effort/loss, verification and execution effort, rework risk, expected benefit and independent-review benefit. Qualitative levels are `low`, `moderate`, `high` or `unknown`; do not turn missing facts into favorable values.

Old `facts`/`selectedCandidate` boolean calls are unsupported and receive a migration error. No caller cost claim or attempt/revision flag is accepted. The phase category is not identity. Assessment freshness includes request, generation, phase ID, context bucket, summary, all options, role capabilities and configured relative costs. A new user input (including extension-sent input), an idle trigger-only run, session replacement/fork/resume, `/tree` navigation, explicit phase assessment, or approximate configurable context growth invalidates a permit. Normal continuations within an active agent run retain their request. Concurrent same-state calls deduplicate; late results cannot open a new generation. Unknown context metrics do not invent growth.
...
Each tool result includes `parentContext`: parent model identifier, window size, configured/effective budget, remaining tokens, state (`within`, `near`, `exceeded`, `unknown`), and explicitly unknown child context. Jev receives numeric/categorical telemetry without the parent model identifier; child model identifiers are not requested. Unknown usage/window/model values remain `null`; usage is approximate. Remaining budget floors at zero. Cached results describe the original assessment snapshot; model/window/budget/state changes participate in cache identity alongside the existing token bucket.
