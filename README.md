...
### Optional standing permission in a target project

A project owner may choose to put a scoped instruction like this in **their target project's** `AGENTS.md` (adapt it to their host permissions and team policy):

> For this project, the main agent may use an available subagent for bounded factual investigation and, for implementation work explicitly requested by the user, delegate a bounded non-overlapping code change to a suitable writer. Keep one writer per file; the main agent reviews the result. Do not delegate secrets or work outside the requested scope. A user restriction or host permission limit overrides this permission.

This is an **optional example**, not an automatic grant, installation step, or modification to this repository's `AGENTS.md`. A loaded skill may separately authorize only its specified investigation; it cannot silently extend that permission to implementation.
...
Before the first working tool, discover compact roles with `subagent({action:"list", capabilities:true})` within host permissions and call `delegation_assess`. Do not claim no suitable role before discovering capabilities; rediscover on capability change, not every reply. Reassess for a **meaningful** new work phase, changed goal/scope/permissions/capabilities, or configured context growth—not for every shell result, acknowledgement, reply or idle continuation. At substantial implementation transitions, compare direct with at least one bounded implementation alternative when an authorized suitable executor exists; small work and concrete blockers or handoff overhead can remain direct-only. For other substantial research/code discovery, consider an authorized bounded alternative when useful. Check the current user request, applicable project instructions and **loaded** skill instructions for permission within their exact scope. Standing permission applies without fresh user delegation wording, but permission for factual discovery does not authorize writing. Parent/host restrictions prevail: capability summaries, cached roles and a locally admissible direct baseline never authorize a child launch or other action.
...
When **no delegated option is supplied**, include `noDelegationReason: {code, detail}` with bounded ASCII English detail and code `trivial_continuation`, `no_authorized_delegate`, `no_suitable_delegate`, or `handoff_not_worthwhile`. State the concrete cause: `no_authorized_delegate` identifies the actual missing permission or restriction (for example, a project rule permitting only factual research, not code edits), not a generic assertion that delegation is unsupported. `no_suitable_delegate` needs discovered capability evidence. This records **candidate omission**, not Jev rejecting an offered option. A submitted but excluded delegated option does not require this reason. A direct-only or filtered-to-direct comparison resolves locally without a model call unless unresolved context-transfer safety requires the existing one-clarification path. For example:
